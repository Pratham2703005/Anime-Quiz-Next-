    ...
    return ( 
        <aside className="relative h-full flex flex-col justify-center text-white shadow-lg opacity-100 px-3 bg-slate-900/50 backdrop-blur-sm border-l border-purple-500/30"> 
            ...
            <div className="flex-grow flex flex-col justify-between p-2 space-y-2 overflow-y-auto max-h-[calc(98vh-60px)] scrollbar-thin scrollbar-thumb-purple-500 scrollbar-track-slate-800"> 
                {moneyLadder.slice().reverse().map((amount: string, index: number) => { 
                    ...
                    return ( 
                        ...
                        > 
                            <div 
                                className={`h-7 flex items-center justify-between px-4 rounded-sm shadow-md transition-all duration-300 ease-in-out border ${(isWin && index===16) ? "bg-gradient-to-r from-yellow-400 to-yellow-300 border-yellow-500":
                                    isCurrent 
